...
async function main() {
  ...
  assert(migrations.length === 5, "discoverMigrations finds exactly the 5 real migrations (001_identity_tenant_core, 002_operations_domain, 003_licensing_domain, 004_administration_domain, 005_cloud_backup_domain — RC1 Sprint 3 added the fifth)");
  ...
  assert(migrations[4].version === '005' && migrations[4].name === 'cloud_backup_domain', 'migration 005 is discovered with the correct name');
  ...
  if (!health.ok) {
    ...
    try {
      ...
      await conn.release();
      assert(applied.size === 5, 'schema_migrations records exactly 5 applied migrations (001, 002, 003, 004, 005 — RC1 Sprint 3 added the fifth)');

      // migrateDown(1) reverts the MOST RECENTLY applied migration first —
      // with 5 migrations now applied, that's 005 (cloud_backup_domain),
      // then 004 (administration_domain), then 003, then 002, then 001.
      const downResult0 = await migrateDown(pool, 1);
      assert(downResult0.length === 1 && downResult0[0].version === '005', 'migrateDown(1) reverts the most-recently-applied migration (005_cloud_backup_domain) first');

      const downResult1 = await migrateDown(pool, 1);
      assert(downResult1.length === 1 && downResult1[0].version === '004', 'a second migrateDown(1) then reverts 004_administration_domain');

      const downResult2 = await migrateDown(pool, 1);
      assert(downResult2.length === 1 && downResult2[0].version === '003', 'a third migrateDown(1) then reverts 003_licensing_domain');

      const downResult3 = await migrateDown(pool, 1);
      assert(downResult3.length === 1 && downResult3[0].version === '002', 'a fourth migrateDown(1) then reverts 002_operations_domain');

      const downResult4 = await migrateDown(pool, 1);
      assert(downResult4.length === 1 && downResult4[0].version === '001', 'a fifth migrateDown(1) then reverts 001_identity_tenant_core, completing a full rollback');
    } finally {
      await closePool();
    }
  }
