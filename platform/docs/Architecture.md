# Z-SUPERADMIN — Platform Foundation & ShopERP Replacement

Z-SUPERADMIN is a completely independent Node/Express/SQLite service (`platform/`) — its own `package.json`, own `node_modules`, own database file (`platform.db`), own JWT secret, own process, own port (default 4100). It shares **nothing** at runtime with `server/local.js` (ShopERP) or any future product. This document is the durable reference, updated for the "v2.0 Platform Replacement" mission — an internal codename for the initiative that replaced ShopERP's own Super Admin console, unrelated to Z-SUPERADMIN's own package version (currently `1.0.0`; see `platform/CHANGELOG.md`): ShopERP's own global Super Admin console has been **permanently removed** from `app/ShopERP_Pro_v8.html`, and Z-SUPERADMIN is now the single, sole control center for it, via the adapter pattern described below. There must never be another Super Admin inside any ZMAX product — see "Adapter Pattern" and "Migration Report".
