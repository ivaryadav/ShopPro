# Architecture — Identity & Tenant Core (Phase 2) + Operations Domain (Phase 4) + Cutover Readiness (Phases 5–6) + Licensing Domain (RC1 Sprint 1)

Status: `server/src/` now contains a real, working, MariaDB-backed implementation of the Identity & Tenant Core bounded context (Phase 2), the Operations domain (Phase 4 — Inventory, Customer, Sale/SaleItem, Repair/RepairPart, Expense, RecurringExpense, StockMovement, Payment; Configuration stays JSON per ADR-0008), and the Licensing domain (RC1 Sprint 1 — Subscription Plans, Tenant Licenses, License History; see `docs/architecture/Licensing.md` for the full layer detail). **It is not the running production server** — `server/local.js` remains that until a future cutover phase (`docs/adr/0001-enterprise-reconstruction.md`, Phase 9). Nothing here is wired into `local.js` or `app/ShopERP_Pro_v8.html`'s main flows; both are unchanged except for a small number of necessary additions (see "Frontend changes" below).
