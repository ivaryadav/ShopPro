...
- Migrated ONLY the Cloud Backup domain into `server/src/`: `POST /api/cloud/backup` (upsert, one slot per license-key hash, no history — matches `local.js` exactly), `GET /api/cloud/restore/:keyHash`, `DELETE /api/cloud/backup/:keyHash` (unconditional, no existence check, preserved as-is) — a new `cloud_backups` table (migration 005), the first table in this engagement with **no `tenant_id` column at all** (keyed purely by license-key hash, ADR-0003). Gated by Sprint 2's `requireAdminSession`, reused unmodified. Used `LONGTEXT` instead of a literal `TEXT` port to avoid MariaDB's 65KB cap silently truncating backup blobs.
- Also delivered `databaseBackupService.js` + `backupVerify.js` — a NEW MariaDB-native operational backup/verify CLI tool, the analog of the pre-existing SQLite `server/scripts/backup-verify.js`: `mysqldump`-based creation, restore-into-a-scratch-database verification (`CHECK TABLE` + row-count comparison), same 0/1 exit-code contract. A genuine addition, not a port — no `local.js` equivalent exists for the MariaDB engine, and the mission's data-safety verification requirements apply to the database this engagement is migrating TO.
- **Real bug found and fixed** by this sprint's real-database, real-`mysqldump` integration testing: `verifyBackup` initially compared restored tables only against their own individual checks, never against the live database's full table set — a truncated/corrupted dump that dropped whole tables entirely still reported `ok: true`, since every table that DID restore passed its own check. Reproduced with a deliberately-truncated real dump against a real disposable MariaDB instance; fixed by adding an explicit live-vs-restored table-set comparison.
- 28 new test assertions (9 mocked + 11 real-database HTTP-domain integration + 8 real-database/real-`mysqldump` Part B integration, including the corruption-detection regression test), zero regressions across the full `test:src` + legacy `server/test/` suites + lint.
- Full detail, including the ADR-0002-vs-ADR-0003 scope reasoning for this structurally different table and every documented deviation: `docs/architecture/Backup.md`.
